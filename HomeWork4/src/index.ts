...
interface BaseIndexType {
  [key: string]: string;
}

interface ExtendedIndexType extends BaseIndexType {
  name: string;
  sureName: string;
}
...
interface CheckVakueType {
  [key: string]: (value: any) => boolean;
}

function validationObjectValue(object: ObjectType, checkValue: CheckVakueType): boolean {
  for (const key in checkValue) {
    if (object.hasOwnProperty(key) && checkValue.hasOwnProperty(key)) {
      const isValid = checkValue[key](object[key]);
